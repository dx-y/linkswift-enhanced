...
// @resource SwalLigt https://cdn.jsdelivr.net/npm/sweetalert2@11.4.8/dist/sweetalert2.min.css
// @resource SwalDark https://cdn.jsdelivr.net/npm/@sweetalert2/theme-dark@5.0.26/dark.min.css
...
// @compatible	      Safari
// @compatible	      Opera
// @downloadURL https://raw.githubusercontent.com/dx-y/linkswift-enhanced/main/linkswift-enhanced.user.js
...
(function linkSwift($) {
	...
	let downloadHistory = {
		STORAGE_KEY: "LinkSwift_download_history",
		MAX_RECORDS: 500,
		...
		addRecord(filename, filesize, downloadMethod, savePath = "") {
		...
		searchHistory(keyword) {
			...
			return history.filter(record => {
				return record.filename.toLowerCase().includes(keyword) ||
					record.downloadMethod.toLowerCase().includes(keyword) ||
					record.savePath.toLowerCase().includes(keyword);
			});
		},
		...
		filterByMethod(method) {
			...
			return history.filter(record => record.downloadMethod === method);
		},
		...
		getMethodIcon(method) {
			const icons = {
				"IDM": '<svg class="pl-icon"><use xlink:href="#pl-icon-fa-download"/></svg>',
				"Aria2": '<svg class="pl-icon"><use xlink:href="#pl-icon-fa-server"/></svg>',
				"Bitcomet": '<svg class="pl-icon"><use xlink:href="#pl-icon-fa-bolt"/></svg>',
				"ABDM": '<svg class="pl-icon"><use xlink:href="#pl-icon-fa-cloud-arrow-down"/></svg>',
				"cURL": '<svg class="pl-icon"><use xlink:href="#pl-icon-fa-terminal"/></svg>',
				"浏览器": '<svg class="pl-icon"><use xlink:href="#pl-icon-fa-globe"/></svg>'
			};
			return icons[method] || '<svg class="pl-icon"><use xlink:href="#pl-icon-fa-file"/></svg>';
		},

		generateHistoryHTML(history, searchKeyword = "", methodFilter = "all") {
			let isEmpty = history.length === 0;
			let emptyMessage = isEmpty ? (searchKeyword ? '，未找到匹配的结果' : '，点击下方按钮刷新页面后重新尝试') : '';

			let html = `<div class="pl-history-container">
				<div class="pl-history-toolbar">
					<div class="pl-history-search">
						<svg class="pl-icon"><use xlink:href="#pl-icon-fa-magnifying-glass"/></svg>
						<input type="text" class="pl-input listener-history-search" placeholder="搜索文件名、方法或路径..." value="${searchKeyword}">
					</div>
					<select class="swal2-select pl-input listener-history-method-filter" style="min-width:120px;">
						<option value="all" ${methodFilter === 'all' ? 'selected' : ''}>全部方式</option>
						<option value="IDM" ${methodFilter === 'IDM' ? 'selected' : ''}>IDM</option>
						<option value="Aria2" ${methodFilter === 'Aria2' ? 'selected' : ''}>Aria2</option>
						<option value="Bitcomet" ${methodFilter === 'Bitcomet' ? 'selected' : ''}>Bitcomet</option>
						<option value="ABDM" ${methodFilter === 'ABDM' ? 'selected' : ''}>ABDM</option>
						<option value="cURL" ${methodFilter === 'cURL' ? 'selected' : ''}>cURL</option>
						<option value="浏览器" ${methodFilter === '浏览器' ? 'selected' : ''}>浏览器</option>
					</select>
					<button type="button" class="pl-button-mini swal2-deny swal2-styled listener-history-clear" title="清空历史记录">
						<svg class="pl-icon"><use xlink:href="#pl-icon-fa-trash"/></svg>
					</button>
				</div>
				<div class="pl-history-stats">
					共 ${history.length} 条记录
				</div>
				<div class="pl-history-list">
					${isEmpty ? `<div class="pl-history-empty">
						<svg class="pl-icon" style="width:48px;height:48px;opacity:0.3"><use xlink:href="#pl-icon-fa-inbox"/></svg>
						<p>暂无下载记录${emptyMessage}</p>
					</div>` : history.map(record => `
						<div class="pl-history-item" data-id="${record.id}">
							<div class="pl-history-item-icon">
								${this.getMethodIcon(record.downloadMethod)}
							</div>
							<div class="pl-history-item-info">
								<div class="pl-history-item-filename" title="${record.filename}">${record.filename}</div>
								<div class="pl-history-item-meta">
									<span class="pl-history-item-method">${record.downloadMethod}</span>
									<span class="pl-history-item-size">${record.filesizeFormatted}</span>
									${record.savePath ? `<span class="pl-history-item-path" title="${record.savePath}">${record.savePath}</span>` : ''}
								</div>
								<div class="pl-history-item-time">${record.date}</div>
							</div>
							<button type="button" class="pl-button-mini swal2-deny swal2-styled listener-history-delete" title="删除此记录" data-id="${record.id}">
								<svg class="pl-icon"><use xlink:href="#pl-icon-fa-x-mark"/></svg>
							</button>
						</div>
					`).join('')}
				</div>
		...
		showHistory(searchKeyword = "", methodFilter = "all") {
			let history = searchKeyword ? this.searchHistory(searchKeyword) : this.getHistory();
			if (methodFilter && methodFilter !== "all") {
				history = history.filter(r => r.downloadMethod === methodFilter);
			}

			const html = this.generateHistoryHTML(history, searchKeyword, methodFilter);
			...
			Swal.fire({
				...
				width: "800px",
				customClass: {
					popup: "pl-popup pl-history-popup",
					content: "pl-content"
				},
				showCloseButton: true,
				showConfirmButton: false,
				showDenyButton: true,
				denyButtonText: '<svg class="pl-icon"><use xlink:href="#pl-icon-fa-trash"/></svg> 清空全部',
				denyButtonColor: "#dc3545",
				didOpen: () => {
					$(".listener-history-search").val(searchKeyword);
					$(".listener-history-method-filter").val(methodFilter);

					$doc.on("input", ".listener-history-search", (e) => {
						const keyword = $(e.target).val();
						const method = $(".listener-history-method-filter").val();
						this.showHistory(keyword, method);
					});

					$doc.on("change", ".listener-history-method-filter", (e) => {
						const method = $(e.target).val();
						const keyword = $(".listener-history-search").val();
						this.showHistory(keyword, method);
					});

					$doc.on("click", ".listener-history-delete", (e) => {
					...
					$doc.on("click", ".listener-history-refresh", (e) => {
						const keyword = $(".listener-history-search").val();
						const method = $(".listener-history-method-filter").val();
						this.showHistory(keyword, method);
						message.info("已刷新");
		...
		testAddRecord() {
			this.addRecord("测试文件.txt", 1024 * 1024, "IDM", "D:\\Downloads");
			message.success("测试记录已添加！请查看下载历史。");
		}
	};
	...
	let base = {
		...
		registerMenuCommand() {
			...
			GM_registerMenuCommand("📋 下载历史", () => {
				downloadHistory.showHistory();
			});
			GM_registerMenuCommand("🧪 测试历史", () => {
				downloadHistory.testAddRecord();
			});
		},
		 ...
		 * @param {String} filename - 文件名
		 * @param {Array} [headers] - 自定义请求头参数（可选）
		 * @param {String} [subDir] - 子目录路径（可选，用于文件夹下载时将文件保存到指定子目录）
		 * @returns {Promise<"success"|"fail">} 发送态结果
		 */
		async sendLinkToAria2(link, filename, headers, subDir) {
			...
			return this.sendLinkToAria2.lock = this.sendLinkToAria2.lock.then(async () => {
				...
				if (subDir && subDir.trim() !== "") {
					let trimedSubDir = subDir.trim();
					let isValidPath = /^[A-Za-z]:[\\/].*$|^[^:*?"<>|]+$/.test(trimedSubDir);
					if (!isValidPath) {
						message.warning("路径包含非法字符，将下载到默认目录");
						dir = undefined;
					} else {
						if (dir && !/^[A-Za-z]:[\\/]/.test(trimedSubDir)) {
							dir = dir.replace(/[\\/]+$/, "") + "/" + trimedSubDir;
						} else {
							dir = trimedSubDir;
						}
					}
		...
		/**
		 * 文件过滤功能
		 * @description 根据文件名、大小、类型过滤文件
		 * @param {String} filename - 文件名
		 * @param {Number} filesize - 文件大小（字节）
		 * @returns {Boolean} 是否通过过滤
		 */
		filterFile(filename) {
			let enableNameFilter = base.getValue("setting_filter_name_exclude");
			let enableTypeFilter = base.getValue("setting_filter_type_exclude");
			let nameFilter = $(".listener-filter-name").val() || $(".listener-aria2-filter-name").val() || "";
			let typeFilter = $(".listener-filter-type").val() || $(".listener-aria2-filter-type").val() || "";

			if (enableNameFilter && nameFilter.trim()) {
				let keywords = nameFilter.split(",").map(k => k.trim()).filter(k => k);
			...
			if (enableTypeFilter && typeFilter.trim()) {
				let extensions = typeFilter.split(",").map(e => e.trim().toLowerCase()).filter(e => e);
		...
		/**
		 * 重命名单个文件
		 * @description 支持四种重命名模式：前缀、后缀、查找替换、正则替换
		 * @param {String} filename - 原始文件名
		 * @param {String} mode - 重命名模式：prefix, suffix, replace, regex
		 * @param {String} pattern - 前缀/查找内容/正则表达式
		 * @param {String} replacement - 替换为（仅替换模式使用）
		 * @returns {String} 新文件名
		 */
		renameFile(filename, mode, pattern, replacement) {
			if (!filename || !pattern) return filename;
			...
			switch (mode) {
				...
				case "suffix":
					return name + pattern + ext;
				case "replace":
					return filename.replace(new RegExp(pattern, "g"), replacement || "");
				case "regex":
					try {
		...
		initDefaultConfig() {
			...
			let defaults = [
				...
				{
					name: "setting_filter_name_exclude",
					value: false
				},
				{
					name: "setting_filter_type_exclude",
					value: true
				}
			];
		...
		showSetting(event) {
			let setting = $(`<div>
				...
				<label class="pl-setting-item" style="display:flex;justify-content:space-between;align-items:center;">
					<div class="listener-tip" data-title="启用后，下载时会根据文件名关键字进行过滤排除">文件名关键字排除</div>
					<label class="pl-switch"><input type="checkbox" class="listener-filter-name-toggle" ${base.getValue("setting_filter_name_exclude") ? "checked" : ""}/><span></span></label>
				</label>
				<label class="pl-setting-item" style="display:flex;justify-content:space-between;align-items:center;">
					<div class="listener-tip" data-title="启用后，下载时会根据文件格式进行过滤排除">文件格式排除</div>
					<label class="pl-switch"><input type="checkbox" class="listener-filter-type-toggle" ${base.getValue("setting_filter_type_exclude") ? "checked" : ""}/><span></span></label>
				</label>
			...
			Swal.fire({
				...
				didOpen: (toast) => {
					...
					element.find(".listener-filter-name-toggle").on("change", function () {
						base.setValue("setting_filter_name_exclude", $(this).is(":checked"));
					});
					element.find(".listener-filter-type-toggle").on("change", function () {
						base.setValue("setting_filter_type_exclude", $(this).is(":checked"));
					});
				},
				willClose: () => {
		...
		async showUpdate() {
			await Swal.fire({
				...
				<div class="block">
					<name>V1.1.15</name>
					<div>
					<div>1、修复 - 夸克网盘推送 IDM 偶发「未启动」（双安装冲突导致会话错位），优化检测到 IDM 实例异常时自动切换可用实例。</div>
					</div>
				</div>
								<div class="block">
					<name>V1.1.14</name>
...
<div class="block">
		...
		generateDom(configs) {
			...
			list.forEach((v, i) => {
				...
				// 应用过滤
				if (!base.filterFile(filename)) return;
				...
				if (!dlink || !dlink.includes("http")) {
					...
					if (temp.mode === "aria2") {
						...
						content.find(".pl-main").append(`<div class="pl-item">
							<div class="pl-item-name listener-tip" data-size="${size}"><div class="name">${filename}</div><div class="size">${base.sizeFormat(size)}</div></div>
							<button class="pl-item-link pl-btn-primary pl-btn-default listener-aria2-download" data-filename="${filename}" data-path="${v.path || ''}" data-link="${dlink}" data-filesize="${size}"><svg class="pl-icon"><use xlink:href="#pl-icon-fa-cloud-arrow-up"/></svg><span>推送链接到 Aria2 下载器</span></button>
			...
			let enableTypeFilter = base.getValue("setting_filter_type_exclude");
			if (enableNameFilter) {
				content.find(".pl-extra").append(`<label class="pl-setting-item" style="display:flex;align-items:center;gap:8px;margin-bottom:10px;">
					<div style="white-space:nowrap;">排除关键字</div>
					<input type="text" autocomplete="off" placeholder="输入要排除的关键字，逗号分隔" class="swal2-input pl-input listener-filter-name" style="flex:1;">
				</label>`);
			}
			if (enableTypeFilter) {
				content.find(".pl-extra").append(`<label class="pl-setting-item" style="display:flex;align-items:center;gap:8px;margin-bottom:10px;">
					<div style="white-space:nowrap;">排除类型</div>
					<input type="text" autocomplete="off" placeholder="输入要排除的扩展名，逗号分隔，如：txt,log" class="swal2-input pl-input listener-filter-type" style="flex:1;">
				</label>`);
			}
			
			// 添加批量重命名功能
			content.find(".pl-extra").append(`
				<div class="pl-rename-section" style="margin-top:15px;padding-top:15px;border-top:1px solid #eee;">
					<div style="font-weight:bold;margin-bottom:12px;">批量重命名</div>
					<div style="display:flex;flex-wrap:wrap;gap:15px;margin-bottom:12px;">
						<label class="pl-setting-item" style="display:flex;align-items:center;gap:8px;">
							<input type="radio" name="rename-mode" value="prefix" checked style="cursor:pointer;">
							<span style="white-space:nowrap;">添加前缀</span>
						</label>
						<label class="pl-setting-item" style="display:flex;align-items:center;gap:8px;">
							<input type="radio" name="rename-mode" value="suffix" style="cursor:pointer;">
							<span style="white-space:nowrap;">添加后缀</span>
						</label>
						<label class="pl-setting-item" style="display:flex;align-items:center;gap:8px;">
							<input type="radio" name="rename-mode" value="replace" style="cursor:pointer;">
							<span style="white-space:nowrap;">查找替换</span>
						</label>
						<label class="pl-setting-item" style="display:flex;align-items:center;gap:8px;">
							<input type="radio" name="rename-mode" value="regex" style="cursor:pointer;">
							<span style="white-space:nowrap;">正则替换</span>
						</label>
					</div>
					<div class="rename-inputs" style="display:flex;flex-direction:column;gap:8px;margin-bottom:12px;">
						<input type="text" autocomplete="off" placeholder="前缀/查找内容/正则表达式" class="swal2-input pl-input listener-rename-pattern" style="width:100%;">
						<input type="text" autocomplete="off" placeholder="替换为（替换模式时使用）" class="swal2-input pl-input listener-rename-replacement" style="width:100%;display:none;">
					</div>
					<div style="display:flex;flex-wrap:wrap;gap:10px;margin-bottom:12px;">
						<button class="pl-btn-primary pl-btn-default listener-rename-preview" style="padding:6px 16px;font-size:13px;"><svg class="pl-icon"><use xlink:href="#pl-icon-fa-eye"/></svg>预览效果</button>
						<button class="pl-btn-primary pl-btn-success listener-rename-apply" style="padding:6px 16px;font-size:13px;"><svg class="pl-icon"><use xlink:href="#pl-icon-fa-check"/></svg>应用重命名</button>
						<button class="pl-btn-primary pl-btn-danger listener-rename-reset" style="padding:6px 16px;font-size:13px;"><svg class="pl-icon"><use xlink:href="#pl-icon-fa-rotate-left"/></svg>重置</button>
					</div>
					<div class="pl-rename-preview-area" style="margin-top:12px;padding:12px;background:#f8f9fa;border-radius:5px;display:none;max-height:200px;overflow-y:auto;">
						<div style="font-weight:bold;margin-bottom:8px;">预览结果：</div>
						<div class="pl-rename-preview-list"></div>
					</div>
				</div>
			`);
			...
			if (temp.mode === "api" && list.length >= 2) {
				...
				content.find(".pl-extra").append(`<div style="display:flex;flex-wrap:wrap;gap:10px;">
					<button class="pl-btn-primary api listener-download-all enhance listener-tip" data-title="通过脚本跨域请求下载文件，已支持多线程、智能多分片，显示预估剩余时间、下载速度；<br/>具体线程取决于浏览器的限制，所以非<b>必要情况（例如系统环境无法安装程序）</b>下，不建议使用此功能!"><svg class="pl-icon"><use xlink:href="#pl-icon-fa-downward"/></svg>全部增强下载 (Beta)</button>
					...
					<button class="pl-btn-primary pl-btn-warning idm listener-open-idm-setting listener-tip" data-title="${rpc.id}" data-back-to-downloads="true"><svg class="pl-icon"><use xlink:href="#pl-icon-fa-gear"/></svg>修改服务参数</button>
				...
				content.find(".pl-extra").append(`<div style="display:flex;flex-direction:column;gap:10px;margin-bottom:12px;">
					<label class="pl-setting-item aria2" style="display:flex;align-items:center;gap:8px;">
						<div style="white-space:nowrap;width:80px;">保存路径</div>
						<input type="text" autocomplete="off" placeholder="输入子目录路径，如：MyFolder/SubFolder，留空则使用默认路径" class="swal2-input pl-input listener-aria2-folder" style="flex:1;">
					</label>
					<label class="pl-setting-item aria2" style="display:flex;align-items:center;gap:8px;">
						<div style="white-space:nowrap;width:80px;">保留结构</div>
						<input type="checkbox" class="listener-aria2-keep-structure" style="width:18px;height:18px;cursor:pointer;" title="保留云盘中的文件夹结构，下载到本地时会自动创建对应的文件夹">
					</label>
				</div>`);
				content.find(".pl-extra").append(`<div style="display:flex;flex-wrap:wrap;gap:10px;">
					<button class="pl-btn-primary pl-btn-warning aria2 listener-open-aria2-setting listener-tip" data-title="${rpc.domain + ":" + rpc.port + rpc.path}" data-back-to-downloads="true"><svg class="pl-icon"><use xlink:href="#pl-icon-fa-gear"/></svg>修改服务参数</button>
					${list.length >= 2 ? `<button class="pl-btn-primary pl-btn-default aria2 listener-send-rpc" data-type="aria2"><svg class="pl-icon"><use xlink:href="#pl-icon-fa-cloud-arrow-up"/></svg>全部推送至下载器</button>` : ""}
					${list.length >= 2 ? `<button class="pl-btn-primary pl-btn-info aria2 listener-copy listener-tip" data-copy='${allLink}' data-title="Aria2 没启用 RPC？点击复制 aria2c 命令行手动下载"><svg class="pl-icon"><use xlink:href="#pl-icon-fa-copy"/></svg>复制全部命令行</button>` : ""}
				</div>`);
		...
		addPageListener() {
			...
			$doc.on("click", ".listener-open-history", () => {
				downloadHistory.showHistory();
			});
			...
			$doc.on("click", ".listener-send-rpc", async function (e) {
				...
				let filteredCount = 0;
				let totalCount = 0;
				$(`.listener-${type}-download`).each((index, element) => {
					let button = $(element);
					if (button.attr("data-processing") !== "true") {
						let filename = button.data("filename");
						let filesize = parseInt(button.data("filesize")) || 0;
						if (type === "aria2" && !base.filterFile(filename)) {
							return;
						}
						filteredCount++;
						button.click();
				...
				if (filteredCount === 0) {
					target.text("没有符合条件的文件").animate({ opacity: "0.5" }, "slow");
				} else {
					target.text(`发送完成，共推送 ${filteredCount} 个文件`).animate({ opacity: "0.5" }, "slow");
				}
				await base.sleep(2000);
				target.css("opacity", "");
				target.html(originalHtml);
			...
			$doc.on("click", ".pl-button-mode", async function (e) {
				...
				// 转存模式单独处理
				if (temp.mode === "transfer") {
					// 使用 api 模式获取链接（与API下载完全相同）
					temp.mode = "api";
					Swal.fire({
						...temp.swalDefault,
						showConfirmButton: false,
						...
						willOpen: async () => {
							...
							try {
								...
								console.log("temp.links:", temp.links);
								console.log("temp.links[0]:", temp.links[0]);
								console.log("temp.links[1]:", temp.links[1]);
								
								let files = temp.links[0].filter(v => !temp.links[1].isFolder || !temp.links[1].isFolder(v));
								console.log("filtered files:", files);
								
								let links = files.map(v => temp.links[1].getFileLink(v)).filter(link => link && link.includes("http"));
								
								console.log("found links:", links);
								
								if (links.length === 0) {
									Swal.close();
									message.warning("没有可转存的文件，请检查是否正确选择了文件");
									return;
								}
								
								// 保存所有链接到GM_setValue（这样跨页面也能访问）
								GM_setValue("linkswift_transfer_links", links);
								GM_setValue("linkswift_transfer_time", Date.now());
								...
								Swal.fire({
									...
									html: `
										<div style="text-align:left;">
											<p style="margin-bottom:12px;">已获取 ${links.length} 个文件链接</p>
											<div style="display:flex;flex-direction:column;gap:10px;">
												<button class="swal2-styled swal2-confirm" style="width:100%;background:#52c41a;border-color:#52c41a;" id="transfer-baidu-btn">
													<span style="font-size:18px;">📦</span> 转存到百度网盘
												</button>
												<button class="swal2-styled swal2-confirm" style="width:100%;background:#1890ff;border-color:#1890ff;" id="transfer-123pan-btn">
													<span style="font-size:18px;">💾</span> 转存到123网盘
												</button>
											</div>
											<div style="margin-top:16px;padding:12px;background:#f5f5f5;border-radius:6px;font-size:12px;color:#666;">
												<strong>操作说明：</strong>
												<ul style="margin:8px 0 0 16px;padding:0;">
													<li>百度网盘：点击"云添加"→粘贴链接→确定</li>
													<li>123网盘：点击"离线下载"→粘贴链接→确定</li>
												</ul>
												<p style="margin-top:8px;"><strong>脚本会自动检测并协助您操作！</strong></p>
											</div>
										</div>
									`,
									showConfirmButton: false,
								...
								setTimeout(() => {
									$("#transfer-baidu-btn").on("click", function () {
										GM_setValue("linkswift_transfer_target", "baidu");
										Swal.fire({
											title: "正在跳转...",
											text: "即将跳转到百度网盘",
											icon: "info",
									...
									$("#transfer-123pan-btn").on("click", function () {
										GM_setValue("linkswift_transfer_target", "123pan");
										Swal.fire({
											title: "正在跳转...",
											text: "即将跳转到123网盘",
											icon: "info",
			...
			// 批量重命名功能
			$doc.on("change", 'input[name="rename-mode"]', function () {
				let mode = $(this).val();
				if (mode === "replace" || mode === "regex") {
					$(".listener-rename-replacement").show();
				} else {
					$(".listener-rename-replacement").hide();
				}
			});
			
			$doc.on("click", ".listener-rename-preview", function () {
				let mode = $('input[name="rename-mode"]:checked').val();
				let pattern = $(".listener-rename-pattern").val();
				let replacement = $(".listener-rename-replacement").val();
				...
				let previewList = [];
				temp.links[0].forEach(v => {
					if (temp.links[1].isFolder && temp.links[1].isFolder(v)) return;
					let originalName = temp.links[1].getFileName(v);
					let newName = base.renameFile(originalName, mode, pattern, replacement);
					if (newName !== originalName) {
						previewList.push({ original: originalName, new: newName });
					}
				});
				
				if (previewList.length === 0) {
					message.info("没有匹配的文件需要重命名");
					return;
				}
				
				let previewHtml = previewList.map(item => 
					`<div style="padding:4px 0;border-bottom:1px dashed #ddd;">
						<span style="color:#666;text-decoration:line-through;">${item.original}</span>
						<span style="margin:0 10px;">→</span>
						<span style="color:#52c41a;font-weight:bold;">${item.new}</span>
					</div>`
				).join("");
				
				$(".pl-rename-preview-list").html(previewHtml);
				$(".pl-rename-preview-area").show();
			});
			
			$doc.on("click", ".listener-rename-apply", function () {
				let mode = $('input[name="rename-mode"]:checked').val();
				let pattern = $(".listener-rename-pattern").val();
				let replacement = $(".listener-rename-replacement").val();
				...
				if (changedCount > 0) {
					message.success(`成功重命名 ${changedCount} 个文件`);
					
					let savePath = $(".listener-aria2-folder").val();
					let keepStructure = $(".listener-aria2-keep-structure").is(":checked");
					...
					setTimeout(() => {
						$('input[name="rename-mode"][value="' + mode + '"]').prop("checked", true);
						$(".listener-rename-pattern").val(pattern);
						$(".listener-rename-replacement").val(replacement);
						$(".listener-aria2-folder").val(savePath);
						$(".listener-aria2-keep-structure").prop("checked", keepStructure);
					...
					message.info("没有文件被重命名");
				}
			});
			
			$doc.on("click", ".listener-rename-reset", function () {
				$('input[name="rename-mode"][value="prefix"]').prop("checked", true);
				$(".listener-rename-pattern").val("");
				$(".listener-rename-replacement").val("");
				$(".listener-rename-replacement").hide();
				$(".pl-rename-preview-area").hide();
			});
		},
...
/* 下载历史记录样式 */
.pl-history-popup{max-width:900px!important;width:900px!important}
.pl-history-container{font-size:13px}
.pl-history-toolbar{display:flex;gap:10px;align-items:center;margin-bottom:15px;padding:10px;background:${temp.color}15;border-radius:8px}
.pl-history-search{flex:1;display:flex;align-items:center;gap:8px;background:#fff;border:1px solid #ddd;border-radius:5px;padding:0 10px}
.pl-history-search .pl-icon{color:#666;flex-shrink:0}
.pl-history-search input{border:none;outline:none;flex:1;font-size:13px;padding:8px 0}
.pl-history-method-filter{min-width:120px}
.pl-history-stats{font-size:12px;color:#666;margin-bottom:10px;padding:0 5px}
.pl-history-list{max-height:500px;overflow-y:auto;padding:5px}
.pl-history-item{display:flex;align-items:center;gap:12px;padding:12px;background:#fff;border:1px solid #eee;border-radius:8px;margin-bottom:8px;transition:all.2s}
.pl-history-item:hover{background:${temp.color}10;border-color:${temp.color}50;box-shadow:0 2px 8px ${temp.color}20}
.pl-history-item-icon{width:40px;height:40px;display:flex;align-items:center;justify-content:center;background:${temp.color}20;border-radius:8px;flex-shrink:0}
.pl-history-item-icon .pl-icon{width:20px;height:20px;color:${temp.color}}
.pl-history-item-info{flex:1;min-width:0}
.pl-history-item-filename{font-weight:500;color:#333;margin-bottom:4px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:14px}
.pl-history-item-meta{display:flex;gap:15px;font-size:12px;color:#666;margin-bottom:4px}
.pl-history-item-method{color:${temp.color};font-weight:500}
.pl-history-item-size{color:#888}
.pl-history-item-path{max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#999}
.pl-history-item-time{font-size:11px;color:#999}
.pl-history-empty{display:flex;flex-direction:column;align-items:center;justify-content:center;padding:50px 20px;color:#999}
.pl-history-empty p{margin-top:15px;font-size:14px}
.pl-history-delete{opacity:0;transition:opacity.2s}
.pl-history-item:hover .pl-history-delete{opacity:1}
@media (max-width:768px){
	.pl-history-popup{max-width:95vw!important;width:95vw!important}
	.pl-history-item-meta{flex-wrap:wrap;gap:8px}
	.pl-history-item-path{display:none}
}
...
`);
	...
	let $baidu = {
		...
		addPageListener() {
			...
			$doc.on("click", ".listener-api-download.enhance", async function (e) {
				...
				base.download(file.link, { "User-Agent": config.$baidu.api.ua.downloadLink, "Origin": "", "Referer": "" }, {
					...
					.then(async (res) => {
						...
						downloadHistory.addRecord(file.name, file.size, "浏览器");
			...
			$doc.on("click", ".listener-aria2-download", async function (e) {
				...
				target.find(".pl-loading").remove();
				target.prepend(base.createLoading());
				let subDir = $(".listener-aria2-folder").val() || "";
				let keepStructure = $(".listener-aria2-keep-structure").is(":checked");
				let relativePath = target.data("path") || "";
				
				let finalSubDir = subDir;
				if (keepStructure && relativePath) {
					let pathParts = relativePath.split("/");
					if (pathParts.length > 1) {
						pathParts.pop();
						let relativeDir = pathParts.join("/");
						finalSubDir = subDir ? `${subDir}/${relativeDir}` : relativeDir;
					}
				}
				
				let res = await base.sendLinkToAria2(target.data("link"), target.data("filename"), [`User-Agent:${config.$baidu.api.ua.downloadLink}`], finalSubDir);
				if (res === "success") {
					downloadHistory.addRecord(target.data("filename"), target.data("filesize"), "Aria2", finalSubDir);
			...
			$doc.on("click", ".listener-bitcomet-download", async function (e) {
				...
				if (res === "success") {
					downloadHistory.addRecord(target.data("filename"), target.data("filesize"), "Bitcomet");
					target.removeClass("pl-btn-danger").html("发送成功啦!快去看看吧~").animate({ opacity: "0.5" }, "slow");
				} else {
					target.addClass("pl-btn-danger").text("发送失败，检查一下您的配置信息哦!").animate({ opacity: "0.5" }, "slow");
				}
			...
			$doc.on("click", ".listener-abdm-download", async function (e) {
				...
				if (res === "success") {
					downloadHistory.addRecord(target.data("filename"), target.data("filesize"), "ABDM");
					target.removeClass("pl-btn-danger").html("发送成功啦!快去看看吧~").animate({ opacity: "0.5" }, "slow");
				} else {
					target.addClass("pl-btn-danger").text("发送失败，检查一下您的配置信息哦!").animate({ opacity: "0.5" }, "slow");
				}
			...
			base.waitForKeyElements(config.$baidu.mount.home, (element) => {
				...
				let $button = $(`<div class="g-dropdown-button pl-button">
					...
					<div class="menu" style="color:${temp.color};border-color:${temp.color};width:auto;z-index:41;">
						...
						<div class="g-button-menu pl-button-mode" data-mode="transfer"><svg class="pl-icon"><use xlink:href="#pl-icon-fa-arrows-rotate"/></svg> 转存到网盘</div>
						...
						<div class="g-button-menu pl-button-mode listener-open-history"><svg class="pl-icon"><use xlink:href="#pl-icon-fa-history"/></svg> 下载历史</div>
						<div class="g-button-menu pl-button-mode" data-mode="transfer"><svg class="pl-icon"><use xlink:href="#pl-icon-fa-arrows-rotate"/></svg> 转存到网盘</div>
					</div>
				</div>`);
				...
				let $button = $(`<div class="wp-s-agile-tool-bar__h-group pl-button">
					<div class="wp-s-agile-tool-bar__h-action is-need-left-sep is-main color-button">
						...
						<ul class="dropdown-list nd-common-float-menu pl-dropdown-menu">
							...
							<li class="sub cursor-p pl-button-mode listener-open-history"><svg class="pl-icon"><use xlink:href="#pl-icon-fa-history"/></svg>下载历史</li>
							<li class="sub cursor-p pl-button-mode" data-mode="transfer"><svg class="pl-icon"><use xlink:href="#pl-icon-fa-arrows-rotate"/></svg>转存到网盘</li>
						</ul>
					</div>
				</div>`);
				element.prepend($button);
	...
	let $aliyun = {
		addPageListener() {
			...
			$doc.on("click", ".listener-aria2-download", async function (e) {
				...
				target.prepend(base.createLoading());
				let subDir = $(".listener-aria2-folder").val() || "";
				let res = await base.sendLinkToAria2(target.data("link"), target.data("filename"), [`Referer:https://${location.host}/`], subDir);
				if (res === "success") {
					downloadHistory.addRecord(target.data("filename"), target.data("filesize"), "Aria2", subDir);
					target.removeClass("pl-btn-danger").html("发送成功啦!快去看看吧~").animate({ opacity: "0.5" }, "slow");
				} else {
					target.addClass("pl-btn-danger").text("发送失败，检查一下您的配置信息哦!").animate({ opacity: "0.5" }, "slow");
				}
	...
	let $mcloud = {
		addPageListener() {
			...
			$doc.on("click", ".listener-aria2-download", async function (e) {
				...
				target.prepend(base.createLoading());
				let subDir = $(".listener-aria2-folder").val() || "";
				let res = await base.sendLinkToAria2(target.data("link"), target.data("filename"), undefined, subDir);
				if (res === "success") {
					downloadHistory.addRecord(target.data("filename"), target.data("filesize"), "Aria2", subDir);
					target.removeClass("pl-btn-danger").html("发送成功啦!快去看看吧~").animate({ opacity: "0.5" }, "slow");
				} else {
	...
	let $tcloud = {
		addPageListener() {
			...
			$doc.on("click", ".listener-aria2-download", async function (e) {
				...
				target.find(".pl-loading").remove();
				target.prepend(base.createLoading());
				let subDir = $(".listener-aria2-subdir").val() || "";
				let res = await base.sendLinkToAria2(target.data("link"), target.data("filename"), undefined, subDir);
				if (res === "success") {
					downloadHistory.addRecord(target.data("filename"), target.data("filesize"), "Aria2", subDir);
					target.removeClass("pl-btn-danger").html("发送成功啦!快去看看吧~").animate({ opacity: "0.5" }, "slow");
	...
	let $quark = {
		addPageListener() {
			...
			$doc.on("click", ".listener-aria2-download", async function (e) {
				...
				let res = await base.sendLinkToAria2(target.data("link"), target.data("filename"), [`User-Agent:${config.$quark.api.ua.downloadLink}`, `Referer:https://${location.host}/`, `Cookie:${document.cookie}`], finalSubDir);
				if (res === "success") {
					downloadHistory.addRecord(target.data("filename"), target.data("filesize"), "Aria2", finalSubDir);
					target.removeClass("pl-btn-danger").html("发送成功啦!快去看看吧~").animate({ opacity: "0.5" }, "slow");
				} else {
		...
		async getLink() {
			...
			if (temp.page === "home") {
				...
				for (let i = 0; i < selectList.length; i += batchSize) {
					...
					if (res?.data) {
						res.data.forEach(item => {
							let originalItem = batch.find(b => b.fid === item.fid);
							if (originalItem && (originalItem.path || originalItem.file_path)) {
								item.path = originalItem.path || originalItem.file_path;
							}
						});
						data.push(...res.data);
					}
				...
				temp.links = [data, {
					...
					getFilePath: v => v.path || v.file_path || "",
					convert: {
	...
	let $uc = {
		addPageListener() {
			...
			$doc.on("click", ".listener-aria2-download", async function (e) {
				...
				target.prepend(base.createLoading());
				let subDir = $(".listener-aria2-subdir").val() || "";
				let res = await base.sendLinkToAria2(target.data("link"), target.data("filename"), [`User-Agent:${config.$uc.api.ua.downloadLink}`, `Referer:https://${location.host}/`, `Cookie:${document.cookie}`], subDir);
				if (res === "success") {
					downloadHistory.addRecord(target.data("filename"), target.data("filesize"), "Aria2", subDir);
					target.removeClass("pl-btn-danger").html("发送成功啦!快去看看吧~").animate({ opacity: "0.5" }, "slow");
				} else {
	...
	(function() {
		...
		function checkTransferTask() {
			let transferLinks = GM_getValue("linkswift_transfer_links");
			let transferTarget = GM_getValue("linkswift_transfer_target");
			let transferTime = GM_getValue("linkswift_transfer_time");
			
			console.log("转存任务检查:", { transferLinks, transferTarget, transferTime });
			
			// 如果没有转存任务，或者任务超过10分钟了，就不处理
			if (!transferLinks || !transferTarget || !transferTime || (Date.now() - transferTime) > 10 * 60 * 1000) {
				return;
			}
			
			// 验证链接数组
			if (!Array.isArray(transferLinks) || transferLinks.length === 0) {
				console.log("链接数组无效");
				return;
			}
			
			// 检查当前页面是否是目标网盘
			let isBaidu = /(pan|yun).baidu.com/.test(location.host);
			let is123pan = /123(pan|684|865|952|912).(com|cn)/.test(location.host);
			
			if ((transferTarget === "baidu" && !isBaidu) || (transferTarget === "123pan" && !is123pan)) {
				return;
			}
			...
			setTimeout(function() {
				Swal.fire({
					...
					html: `
						<div style="text-align:left;">
							<p>发现 ${transferLinks.length} 个待转存的文件！</p>
							<div style="margin-top:12px;padding:10px;background:#f5f5f5;border-radius:6px;">
								<strong>操作说明：</strong>
								<ul style="margin:8px 0 0 16px;padding:0;">
									<li>${transferTarget === "baidu" ? "点击「云添加」按钮" : "点击「离线下载」按钮"}</li>
									<li>脚本会自动把链接复制到剪贴板</li>
									<li>粘贴链接并确定即可</li>
								</ul>
							</div>
							<div style="margin-top:12px;">
								<strong>链接预览（前3个）：</strong>
								${transferLinks.slice(0, 3).map((link, i) => `<div style="margin:4px 0;padding:4px;background:#e8f4fd;border-radius:4px;font-size:12px;word-break:break-all;">${i+1}. ${link.substring(0, 50)}...</div>`).join('')}
								${transferLinks.length > 3 ? `<div style="font-size:12px;color:#666;">...还有 ${transferLinks.length - 3} 个链接</div>` : ''}
							</div>
						</div>
					`,
					icon: "info",
					showCancelButton: true,
					confirmButtonText: "开始协助",
					...
					if (result.isConfirmed) {
						...
						if (transferLinks.length > 0) {
							try {
								await navigator.clipboard.writeText(transferLinks[0]);
							} catch {
								let textarea = document.createElement('textarea');
								textarea.value = transferLinks[0];
						...
						// 尝试自动点击按钮
						await autoClickButton(transferTarget);
						
						// 监控输入框，自动填充
						monitorAndFill(transferLinks, transferTarget);
					} else {
						// 用户取消了，清理数据
						GM_deleteValue("linkswift_transfer_links");
						GM_deleteValue("linkswift_transfer_target");
						GM_deleteValue("linkswift_transfer_time");
					}
				});
			}, 1500);
		}
		...
		// 尝试自动点击按钮
		async function autoClickButton(target) {
			let targetText = target === "baidu" ? "云添加" : "离线下载";
			
			if (target === "baidu") {
				// 百度网盘的"云添加"按钮
				let selectors = [
					".wp-s-agile-tool-bar__btn-wp-new-create",
					".g-button.g-button-blue",
					".wp-s-agile-tool-bar__h-btn",
					"button:contains('云添加')",
					"span:contains('云添加')",
					".g-btn-primary",
					".btn-primary",
					"button[data-action='offline']",
					"button[title*='添加']",
					"button[title*='新建']"
				];
				
				for (let i = 0; i < 15; i++) {
					await base.sleep(400);
					...
					for (let selector of selectors) {
						try {
							...
							for (let btn of buttons) {
								...
								if (text.includes(targetText) || ariaLabel.includes(targetText) || title.includes(targetText)) {
									btn.click();
									toast.fire({
										title: `已自动点击「${targetText}」按钮！`,
										icon: "success",
										timer: 2000
									});
									return;
			...
			} else if (target === "123pan") {
				// 123网盘：先悬停并点击"离线下载"按钮展开下拉菜单，再点击"新建链接下载任务"
				for (let i = 0; i < 15; i++) {
					await base.sleep(300);
					
					// 先查找下拉菜单中的"新建链接下载任务"选项
					let menuItem = document.querySelector(".offline-down-menu-item");
					if (menuItem) {
						menuItem.click();
						toast.fire({
							title: "已自动点击「新建链接下载任务」！",
							icon: "success",
							timer: 2000
						});
						return;
					...
					// 如果菜单没展开，查找"离线下载"按钮并触发悬停和点击
					let offlineBtn = null;
					let buttons = document.querySelectorAll('button, div[role="button"], div.ant-btn');
					for (let btn of buttons) {
						let text = btn.textContent || btn.innerText || "";
						let ariaLabel = btn.getAttribute('aria-label') || "";
						let title = btn.getAttribute('title') || "";
						
						if (text.includes("离线下载") || ariaLabel.includes("离线下载") || title.includes("离线下载")) {
							offlineBtn = btn;
							break;
						}
					}
					...
					if (offlineBtn) {
						...
						offlineBtn.dispatchEvent(hoverEvent);
						...
						offlineBtn.dispatchEvent(mouseOverEvent);
		...
		// 监控并填充链接
		function monitorAndFill(links, target) {
			...
			function findInputBox() {
				...
				// 1. 直接查找123网盘的离线下载输入框
				let textareas = document.querySelectorAll('textarea.ant-input');
				for (let textarea of textareas) {
					let placeholder = textarea.placeholder || "";
					if (placeholder.includes("链接") || placeholder.includes("下载")) {
						return textarea;
					}
				}
				
				// 2. 查找modal中的输入框
				let modalBody = document.querySelector('.ant-modal-body');
				if (modalBody) {
					let modalInputs = modalBody.querySelectorAll('textarea, input');
				...
				let selectors = [
					...
					'textarea[placeholder*="下载"]',
					'input[placeholder*="链接"]',
					'input[placeholder*="下载"]',
					'.ant-input-outlined',
					'.auto-focus-input'
				];
			...
			function findConfirmButton() {
				let selectors = [
					...
					'.ant-btn.css-v0tdpn',
					'.ant-btn.css-var-r_2v_',
					'button.btn.ant-btn',
					'button.mfy-button',
					'.ant-btn-variant-solid.mfy-button',
					'.css-v0tdpn.ant-btn-primary'
				];
				...
				for (let selector of selectors) {
					try {
						...
						for (let btn of buttons) {
							let text = btn.textContent || btn.innerText || "";
							let spanText = btn.querySelector('span')?.textContent || "";
							let ariaLabel = btn.getAttribute('aria-label') || "";
							let title = btn.getAttribute('title') || "";
							
							if (text.includes("确定") || spanText.includes("确定") || 
							    text.includes("确认") || spanText.includes("确认") ||
							    ariaLabel.includes("确定") || ariaLabel.includes("确认") ||
							    title.includes("确定") || title.includes("确认")) {
								return btn;
							}
						}
					} catch {}
				...
				for (let btn of allButtons) {
					let text = btn.textContent || btn.innerText || "";
					if ((text.includes("确定") || text.includes("确认")) && btn.classList.contains('ant-btn')) {
						return btn;
					}
				}
			...
			async function clickConfirmButton() {
				...
				console.log("找到确认按钮:", btn);
				
				try {
					// 先触发鼠标事件
					let mouseEnter = new MouseEvent('mouseenter', { bubbles: true });
			...
			async function setReactInputValue(input, value) {
				...
				try {
					// 方法1：使用TextEncoder和InputEvent（最现代的方式）
					try {
			...
			async function fillNextLink() {
				...
				let link = links[currentLinkIndex];
				...
				if (input) {
					try {
						...
						let success = await setReactInputValue(input, link);
						...
						if (success) {
							...
							toast.fire({
								title: `已填充第 ${currentLinkIndex + 1}/${links.length} 个链接`,
								icon: "success",
								timer: 2000
							});
			...
			let intervalId = setInterval(function() {
				fillNextLink();
				if (currentLinkIndex >= links.length || fillAttempts >= maxAttempts) {
		...
		if (document.readyState === "loading") {
			document.addEventListener("DOMContentLoaded", checkTransferTask);
		} else {
			checkTransferTask();
		}
	})();
